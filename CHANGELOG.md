...
- **Normalização de Layout (Aba Pagamentos)**: Otimização extrema do Hub e cards de métricas. Remoção da métrica redundante "Volume Processado", redução da largura do Hub para 1 coluna e ajuste assimétrico de padding nos cards secundários (`pr-2`). Isso posiciona os títulos e valores monetários mais próximos à borda direita, maximizando a densidade de dados e a clareza visual para executivos.
