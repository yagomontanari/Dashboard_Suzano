...
- **Normalização de Layout (Aba Pagamentos)**: Aplicação do sistema de alinhamento "Grid Shelf" e bordas `rounded-xl` em todos os cards de métricas de pagamento. Altura dos cards reduzida proporcionalmente para `145px` (Métricas) e `165px` (Hub), garantindo paridade estética total com o módulo ZAJU.
