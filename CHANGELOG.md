...
### [2.4.34] - 2026-05-05
- **UX/UI (Dashboard)**: Implementada a nova aba **"Inteligência de Vendas"**, oferecendo uma visão analítica premium de Receita, Margem Bruta e Eficiência de Pagamento.
- **Backend (API)**: Novos endpoints e queries SQL otimizadas para processamento de métricas de Sell-in e performance regional.
...
- **Bug Fix (Backend)**: Corrigido erro `UndefinedFunctionError` no PostgreSQL ao utilizar `SUBSTRING` em parâmetros de data. Implementado `CAST` explícito para `TEXT` nas queries de VK11 para garantir compatibilidade com a inferência de tipos do motor de banco de dados.
...
### Corrigido
- **ZAJU (Performance)**: Otimização crítica da query de rateio pendente. Redução do tempo de carregamento de ~4 minutos para <10 segundos através de filtragem seletiva de produtos e Customer Groups, eliminando travamentos na interface (Loading infinito).
- **Backend (Estabilidade)**: Correção de nomes de colunas e inclusão de type-casts necessários para compatibilidade com o driver asyncpg, garantindo o retorno correto dos dados.
