...
QUERY_ORCAMENTO_INTEGRACAO_TOTAL = text("""
    ...
    FROM (
        ...
        WHERE soi.valid_from <= SUBSTRING(CAST(:end_date AS TEXT) FROM 1 FOR 10)
          AND soi.valid_to >= SUBSTRING(CAST(:start_date AS TEXT) FROM 1 FOR 10)
...
QUERY_DASHBOARD_COUNTS_CONSOLIDATED = text("""
    SELECT
        ...
        (
            ...
            FROM suzano_orcamento_integracao soi
            WHERE soi.status = 'ERRO' AND soi.valid_from <= SUBSTRING(CAST(:end_date AS TEXT) FROM 1 FOR 10) AND soi.valid_to >= SUBSTRING(CAST(:start_date AS TEXT) FROM 1 FOR 10)
        ) as vk11
""")
...
QUERY_ERRO_VK11_LIST = text("""
    ...
    WHERE soi.status = 'ERRO'
      AND soi.valid_from <= SUBSTRING(CAST(:end_date AS TEXT) FROM 1 FOR 10) 
      AND soi.valid_to >= SUBSTRING(CAST(:start_date AS TEXT) FROM 1 FOR 10)
""")
...
QUERY_ZAJU_PENDENTE_SEM_RATEIO = text("""
    WITH pending_memory AS (
        SELECT 
            ...
            (SELECT SUM(vlr_desdobramento) FROM orcamento_desdobramento WHERE id_linha_investimento = oli.id) as cond_value,
            sapmc.dta_criacao,
            sapmc.dta_alteracao,
            ce.id_extensao as id_cg,
            concat(cg.id_externo, ' - ', cg.nom_extensao) as cg_name,
    ...
    SELECT 
        ...
        ap.linha_desc as linha_investimento,
        ap.cg_name as customer_group,
        ap.marca_name as marca,
        ap.cond_value as valor_provisao,
