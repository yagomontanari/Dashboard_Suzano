...
async def login(form_data: LoginRequest, db: AsyncSession = Depends(get_app_db)):
    ...
    if user.locked_until and user.locked_until > datetime.utcnow():
        ...
        raise HTTPException(
            status_code=403,
            detail=f"Conta bloqueada por {minutes} min devido a multiplas tentativas falhas.",
        )
    ...
    if not user.hashed_password or not verify_password(
        ...
        if user.failed_login_attempts >= 5:
            ...
            user.failed_login_attempts = 0  # Opcional: zerar apos bloquear
            await db.commit()
            raise HTTPException(
                status_code=403,
                detail="Conta bloqueada por 1 hora devido a multiplas tentativas falhas.",
            )
        await db.commit()
    ...
    # 4. Verificar Expiracao (90 dias)
    # Se password_updated_at for None (migracao), tratamos como OK e forcamos o update
...
async def register(data: RegisterRequest, db: AsyncSession = Depends(get_app_db)):
    ...
    if result.scalar_one_or_none():
        raise HTTPException(
            status_code=400, detail="Este e-mail ja possui uma solicitacao ou cadastro."
        )
...
async def change_password(
    data: ChangePasswordRequest, token: str, db: AsyncSession = Depends(get_app_db)
):
    ...
    if not user or (
        ...
        and not verify_password(data.old_password, user.hashed_password)
    ):
    ...
    # 2. Validar Historico (Últimas 12)
    ...
    for hist in history_hashes:
        if verify_password(data.new_password, hist.hashed_password):
            raise HTTPException(
                status_code=400,
                detail="A nova senha nao pode ser igual as ultimas 12 senhas utilizadas.",
            )

    # 3. Salvar no Historico antes de atualizar
...
async def list_users(
    db: AsyncSession = Depends(get_app_db), admin: User = Depends(check_admin)
):
...
async def update_user(
    ...
    if user.id == admin.id and data.active is False:
        raise HTTPException(
            status_code=400,
            detail="Você nao pode inativar sua propria conta de administrador.",
        )
...
async def approve_user(
    ...
    # Enviar email de aprovacao com a senha temporaria
    await mail_service.send_approval_email(user.email, user.nome, temp_pwd)

    return {"message": "Usuario aprovado e credenciais enviadas por e-mail!"}
...
async def reject_user(
    ...
    if user:
        ...
        user.active = False
        await db.commit()
    return {"message": "Usuario reprovado."}


@router.post(
    "/admin/users/{user_id}/reset-password", response_model=PasswordResetResponse
)
async def reset_password(
    ...
    # Enviar email de reset (acao do admin)
    await mail_service.send_reset_password_email(user.email, user.nome, temp_pwd)

    return {"message": "Senha resetada com sucesso e enviada ao usuario!"}
...
async def unlock_user(
    ...
    # (Opcional) Notificar usuario do desbloqueio se quiser
...
async def forgot_password(
    data: ForgotPasswordRequest, db: AsyncSession = Depends(get_app_db)
):
    result = await db.execute(select(User).where(User.email == data.email))
    user = result.scalar_one_or_none()

    # Por seguranca, mesmo que o usuario nao exista, nao informamos.
    if not user or not user.active:
        return {
            "message": "Se o e-mail estiver cadastrado, uma nova senha sera enviada."
        }
    ...
    return {"message": "Uma nova senha temporaria foi enviada para o seu e-mail."}
