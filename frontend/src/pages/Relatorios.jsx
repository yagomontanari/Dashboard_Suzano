...
const MonthFilter = ({ value, onChange, iconColor = "text-blue-500", activeColor = "bg-blue-600", label = "Competência" }) => {
  ...
  return (
    <div className="relative flex items-center bg-white p-1 rounded-2xl border border-slate-200 shadow-sm w-full">
      <div className="flex items-center gap-2 px-4 py-2 border-r border-slate-100 shrink-0">
      ...
      <div className="relative flex-1">
        <button 
          ...
          className="w-full flex items-center justify-between gap-3 px-4 py-2 hover:bg-slate-50 rounded-xl transition-all group"
        >
          <span className="text-sm font-bold text-slate-700 capitalize">
            {fullMonths[currentMonthIdx]} <span className="text-slate-400 font-medium ml-1">{currentYear}</span>
          </span>
          ...
          <>
            ...
            <div className="absolute top-full mt-3 right-0 w-72 bg-white border border-slate-200 rounded-3xl shadow-2xl z-30 p-5 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
              <div className="flex items-center justify-between mb-6 px-1">
                <button onClick={() => setViewYear(v => v - 1)} className="p-2 hover:bg-slate-100 rounded-xl text-slate-400 hover:text-blue-600 transition-colors"><ChevronLeft size={18} /></button>
                <span className="text-base font-black text-slate-800 tracking-tight">{viewYear}</span>
                <button onClick={() => setViewYear(v => v + 1)} className="p-2 hover:bg-slate-100 rounded-xl text-slate-400 hover:text-blue-600 transition-colors"><ChevronRight size={18} /></button>
              </div>
              <div className="grid grid-cols-3 gap-2">
                {months.map((m, idx) => (
                  <button
                    ...
                    className={`py-3 rounded-2xl text-xs font-black transition-all ${currentMonthIdx === idx && currentYear === viewYear ? `${activeColor} text-white shadow-lg` : 'hover:bg-slate-50 text-slate-500 hover:text-slate-800'}`}
                  >
                    {m}
                  </button>
                ))}
...
const MonthRangeFilter = ({ start, end, onStartChange, onEndChange, iconColor = "text-orange-500", activeColor = "bg-orange-600", label = "Período" }) => {
  const [isOpen, setIsOpen] = useState(null); // 'start' or 'end' or null
      ...
      <div className="flex items-center flex-1 px-1">
        <div className="relative flex-1">
          <button 
            ...
            className={`w-full flex items-center justify-center gap-2 py-2 hover:bg-slate-50 rounded-xl transition-all text-sm font-bold capitalize ${isOpen === 'start' ? 'text-orange-600 bg-orange-50' : 'text-slate-700'}`}
          >
            {fullMonths[parseInt(start.split('-')[1]) - 1]} <span className="text-slate-400 font-medium">{start.split('-')[0]}</span>
          </button>
          {isOpen === 'start' && (
            <MonthPopover value={start} onChange={(v) => { onStartChange(v); setIsOpen(null); }} onClose={() => setIsOpen(null)} activeColor={activeColor} />
          )}
        </div>
        
        <div className="px-2 text-slate-300 font-bold">/</div>
        
        <div className="relative flex-1">
          <button 
            onClick={() => setIsOpen(isOpen === 'end' ? null : 'end')}
            className={`w-full flex items-center justify-center gap-2 py-2 hover:bg-slate-50 rounded-xl transition-all text-sm font-bold capitalize ${isOpen === 'end' ? 'text-orange-600 bg-orange-50' : 'text-slate-700'}`}
          >
            {fullMonths[parseInt(end.split('-')[1]) - 1]} <span className="text-slate-400 font-medium">{end.split('-')[0]}</span>
          </button>
          {isOpen === 'end' && (
            <MonthPopover value={end} onChange={(v) => { onEndChange(v); setIsOpen(null); }} onClose={() => setIsOpen(null)} activeColor={activeColor} />
          )}
        </div>
      </div>
    </div>
...
const MonthPopover = ({ value, onChange, onClose, activeColor }) => {
  const [viewYear, setViewYear] = useState(parseInt(value.split('-')[0]));
  const months = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];
  const currentMonthIdx = parseInt(value.split('-')[1]) - 1;
  const currentYear = parseInt(value.split('-')[0]);
  ...
  return (
    <>
      ...
      <div className="absolute top-full mt-3 right-0 w-72 bg-white border border-slate-200 rounded-3xl shadow-2xl z-30 p-5 animate-in fade-in zoom-in-95 duration-200 origin-top">
        <div className="flex items-center justify-between mb-6 px-1">
          <button onClick={() => setViewYear(v => v - 1)} className="p-2 hover:bg-slate-100 rounded-xl text-slate-400 transition-colors"><ChevronLeft size={18} /></button>
          <span className="text-base font-black text-slate-800 tracking-tight">{viewYear}</span>
          <button onClick={() => setViewYear(v => v + 1)} className="p-2 hover:bg-slate-100 rounded-xl text-slate-400 transition-colors"><ChevronRight size={18} /></button>
        </div>
        <div className="grid grid-cols-3 gap-2">
          {months.map((m, idx) => (
            <button
              key={m}
              onClick={() => onChange(`${viewYear}-${String(idx + 1).padStart(2, '0')}`)}
              className={`py-3 rounded-2xl text-xs font-black transition-all ${currentMonthIdx === idx && currentYear === viewYear ? `${activeColor} text-white shadow-lg` : 'hover:bg-slate-50 text-slate-500 hover:text-slate-800'}`}
            >
              {m}
            </button>
...
const YearFilter = ({ value, onChange, iconColor = "text-blue-500", activeColor = "bg-blue-600", label = "Anos" }) => {
  ...
  const handleToggle = (year) => {
    ...
    if (value.includes(year)) {
      ...
      newValue = value.length >= 2 ? [value[1], year].sort() : [...value, year].sort();
    }
    onChange(newValue);
  };
          ...
          <>
            ...
            <div className="absolute top-full mt-3 right-0 w-64 bg-white border border-slate-200 rounded-3xl shadow-2xl z-30 p-5 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
              <div className="space-y-2">
                {availableYears.map(year => (
                  <button
                    ...
                    className={`w-full py-3 px-4 rounded-2xl text-xs font-black transition-all flex items-center justify-between ${value.includes(year) ? `${activeColor} text-white shadow-lg` : 'hover:bg-slate-50 text-slate-500 hover:text-slate-800'}`}
                  >
                    {year}
              ...
              <div className="mt-6 pt-4 border-t border-slate-50 flex items-center justify-between">
                <button onClick={() => setIsOpen(false)} className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:text-slate-600">Fechar</button>
            ...
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-12">
              ...
              <div className="space-y-8">
                <div className="space-y-2">
                  {saldosMode === 'mensal' ? (
                    <MonthRangeFilter 
                      start={saldosStartMonth} 
                      end={saldosEndMonth} 
                      onStartChange={setSaldosStartMonth}
                      onEndChange={setSaldosEndMonth}
                      label="Período"
